...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = (
        ...
        f"{'Mini Response':<43} | {'GPT-4o Latency':>13} | {'Mini Latency':>12}"
    )
    separator = "-" * len(header)
    lines = [header, separator]
    for item in results:
        lines.append(
            f"{_short(item.get('prompt', '')):<42} | "
            f"{_short(item.get('gpt4o_response', '')):<43} | "
            f"{_short(item.get('mini_response', '')):<43} | "
            f"{float(item.get('gpt4o_latency', 0.0)):>13.3f} | "
            f"{float(item.get('mini_latency', 0.0)):>12.3f}"
        )
    return "\n".join(lines)
